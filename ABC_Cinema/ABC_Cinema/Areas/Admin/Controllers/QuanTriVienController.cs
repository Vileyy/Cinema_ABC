...
namespace ABC_Cinema.Areas.Admin.Controllers
{
    public class QuanTriVienController : Controller
    {
        ...
        public ActionResult Index(int? page, int? size, string sortProperty, string sortOrder = "", string searchString = null)
        {
            ...
            var result = db.TaiKhoans.Where(p => p.PhanQuyen == "MANAGER");
        ...
        public ActionResult Create(TaiKhoan tk, FormCollection f)
        {
            ...
            var tenTK = f["sTenTK"];
            var bool_Tk = db.TaiKhoans.Any(t => t.TenDangNhap == tenTK && t.PhanQuyen == "MANAGER");

            var email = f["sEmail"];
            var bool_Email = db.TaiKhoans.Any(t => t.Id_ThongTin != null && t.ThongTin.Email == email && t.PhanQuyen == "MANAGER");
        ...
        {
            var maQTV = int.Parse(f["iMaQTV"]);
            var tk = db.TaiKhoans.SingleOrDefault(t => t.Id_TaiKhoan == maQTV);
